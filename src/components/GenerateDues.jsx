...
const GenerateDues = () => {
  ...
  async function onCLick(){
    ...
    {
      toast.error('Please provide the mandatory fields',{position:'top-right'});
      return;
    }
    if(endMonth.value<startMonth.value)
    {toast.error('Start Month is greater than end month',{position:'top-right'});
        return;
    }
    
    try {
      setLoading(true);
      const url=BASE_URL+`/admin/generateDues?startDate=${startMonth.value}&endDate=${endMonth.value}&grade=${(!grade?.value) ?'': grade.value}`;
