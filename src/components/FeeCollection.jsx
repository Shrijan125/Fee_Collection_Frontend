...
const FeeCollection = () => {
  ...
  const submitHandler=async()=>{
    ...
    {
      toast.error('Please fill the mandatory fields!',{position:'top-right'});
      return;
    }
    const url= BASE_URL + `/admin/collectFee`;
    const createMonth = feeForMonth.map(element => element.value);
    try {
      const {data} =await axios.post(url,{admno,month:createMonth,amount,description:note});
  ...
  useEffect(()=>{
    if(feeForMonth.length===0)
      ...
      axios.get(url).then((data)=>{ 
        ...
        setFine(data.data.data.fine);}).catch((error)=>{
        setFirstName('');
        setMiddleName('');
        setLastName('');
        setGrade('');
  ...
  return (
    ...
    <div className="grid grid-cols-2 gap-5 justify-items-center">
      <ButtonWithIcon src={"src/assets/search.png"} onClickHandler={onClickHandler} label={'Admission No.'} value={admno} setValue={setAdmno}></ButtonWithIcon>
        <FormBox
          disabled={true}
          label={'First Name'}
          value={firstName}
        ...
        <FormBox label={'Note'} setField={setNote} value={note}></FormBox>
      </div>
      {(fine!==0 && fine!=='') && <span className='mt-3 text-xl font-bold text-light'>* Late fine applicable: Rs.{fine}</span>}
